...
`modal run modal/release.py::export_and_validate`, then ::export_cpu_check, then ::push_to_hf.
"""
...
def export_and_validate(
    ...
    export.export_and_validate(ckpt, OUT_ROOT, val_subset=val_subset,
                               test_subset=test_subset, on_saved=outputs_vol.commit)
    outputs_vol.commit()
...
def push_to_hf(
    ...
    api.upload_folder(
        ...
        repo_id=repo,
        repo_type="model",
        commit_message="Greyscope v1 — merged weights + calibration",
    )
