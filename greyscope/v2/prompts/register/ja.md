...
日本語の文章の AI 書き換えパターン検出・除去リファレンス。
...
AIは多言語データで学習しているため、カタカナ外来語を多用する。
...
**原則：** 本当に3つなら3つでいい。4つあるなら4つ書く。1つだけでもいい。「3つにまとめると」で始まる文を見たら疑う。
...
**NG例：**
> 単なるツールではない。パラダイムシフトだ。コストの問題ではない。文化の問題だ。

**原則：** 空虚なら削る。AIはこのリズムで劇的さを演出する。ただし正当な修辞でもある（ケネディ「国があなたに何をしてくれるかではなく……」）。対比が本物で必然なら残す。問題は形式ではなく、空疎な反転の繰り返し。
...
**原則：** データの後に「〜を示しており」「〜を物語っています」が続いたら、その文を丸ごと削除する。読者に解釈を委ねる。
...
**注意：** 使い回しの人名、全員が同じ肩書き（みんな「専門家」「博士」）、地の文と同じ口調の引用

**NG例：**
> 著名な専門家の田中博士はこう語る。「これは業界を根本から変えるパラダイムシフトだ。」

**OK例：**
> 整備班をまとめる山本さんは率直だった。「毎週金曜、ガムテープで応急処置してた。」

**原則：** 具体的で文脈に合った、ばらつきのある名前と肩書きを使う。本物の引用はもっと短く、荒く、地の文とは違う声。字数稼ぎの架空の人物や発言は削る。

---

### 22. 短文の連打（スローガン調）

**注意：** 「ムダなし。飾りなし。結果だけ。」「集中。整合。計測可能。」勢いのための単語文の積み重ね（LinkedIn／マーケ調）

**NG例：**
> ムダなし。飾りなし。結果だけ。速い。堅牢。スケールする。

**OK例：**
> 2週間でリリースでき、毎秒1万リクエストでも落ちない。

**原則：** スローガンのリズムを、実際の情報を持つ1文に置き換える。

---

### 23. 問いかけを接続に使う

**注意：** 自問自答を接続詞代わりに：「その結果は？……」「なぜ重要か？……」「落とし穴は？……」

**NG例：**
> パーサを書き直した。その結果は？3倍速くなった。

**OK例：**
> パーサを書き直したら3倍速くなった。

**原則：** 演出的な問いを消して、結果をそのまま書く。本気の問いが1〜2回ならいいが、連発はAIっぽさ。

---

### 24. 冒頭の決まり文句

**注意：** 「変化の激しい現代において」「進化し続ける〜の世界で」「想像してみてほしい。」「〜したことはないだろうか」「さっそく見ていこう」

**NG例：**
> 変化の激しい現代のデジタル社会で、企業は生き残るために常に革新し続けなければならない。

**OK例：**
> 先期、温めていた機能を先に出してきた競合に、顧客を2社取られた。

**原則：** 具体的な事実・場面・利害から始める。前置きの「枕」は丸ごと削る。
