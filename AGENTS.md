...
The shipped v2 model is **trilingual (EN/ja/zh-TW), Apache-2.0**. The previous English-only,
CC BY-NC-SA model is recoverable from git tag `v1.0`. The code is de-versioned (one recipe, one set
of entrypoints); only the dataset artifacts are versioned under `data/v2/`.
...
- **Train view vs release view** — `assemble` keeps every edit for *training* (training ≠
  ...
  `generate.SHIPPABLE_EDIT_SOURCES`), since shipping an edit ships a derivative. A distributable
  dataset release is not built.
...
int4 tracks bf16 at 98.4% bucket agreement. Release artifacts are
`greyscope-v2-qwen3.5-4b`, `greyscope-v2-qwen3.5-4b-int4`, and
`greyscope-v2-qwen3.5-4b-mlx-4bit`; v1's repo is untouched. Modal CLI: this project lives in the
`yaoandy107` workspace — run with `MODAL_PROFILE=yaoandy107`.**
